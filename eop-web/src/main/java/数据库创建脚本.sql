...
CREATE TABLE plan (
   ...
   CONSTRAINT fk_eid30 FOREIGN KEY (eid) REFERENCES emp(eid)
...
CREATE TABLE plan_details (
   ...
   CONSTRAINT fk_eid20 FOREIGN KEY(eid) REFERENCES emp(eid) 
) engine='innodb' ;
...
CREATE TABLE emp_logs (
   elid               bigint AUTO_INCREMENT ,
   eid                varchar(50),
   logintime          datetime,
   constraint PK_elid primary key (elid) ,
   CONSTRAINT fk_eid31 FOREIGN KEY(eid) REFERENCES emp(eid)
) engine=innodb ;
...
-- 增加数据字典信息表
-- 创建用品分类
INSERT INTO dictionary(type,title,value) VALUES ('resource','文具','1') ;
INSERT INTO dictionary(type,title,value) VALUES ('resource','耗材','2') ;
INSERT INTO dictionary(type,title,value) VALUES ('resource','硬件','3') ;
