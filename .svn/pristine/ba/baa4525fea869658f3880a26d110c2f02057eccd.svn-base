...
<%!
	public static final String SUPPLY_EMP_APPLY_HANDLE_URL = "/eop/pages/back/admin/supply/supply_emp_apply_edit.action" ;
%>
				...
				<div>
					<table class="table table-condensed" style="width:700px;">
						...
						<tr>
							...
							<td id="status">${EmpResource.status}</td>
						</tr>
						<tr>
							<td><strong>备注：</strong></td>
							<td><pre class="pre-scrollable" style="width:500px;height:100px;">数据库没有note字段</pre></td>
						</tr>
					</table>
				</div>
						...
						<div class="panel-heading">
							<h4 class="panel-title"> 
								<a data-toggle="collapse" data-parent="news" href="#contentOne">
									<strong><span class="glyphicon glyphicon-tasks"></span>&nbsp;办公用品清单（用品数量：${amount}件）</strong>
								</a>
							</h4>
						</div>
									...
									<c:forEach items="${allResource}" var="resource">
										<tr id="travel-1">
											<td class="text-center">
												<img src="upload/resource/${resource.photo}" style="width:20px;"/> 
											</td>
				...
				<div>
					<form class="form-horizontal" action="<%=SUPPLY_EMP_APPLY_HANDLE_URL%>" id="myform" method="post">
						<fieldset>
							<div class="form-group" id="auditDiv">
								...
								<div class="col-md-6">
									<div class="radio-inline">
										<label><input type="radio" id="status" name="status" value="2" checked>
											&nbsp;<span class="text-danger">拒绝</span></label>
									</div> 
									<div class="radio-inline">
										<label><input type="radio" id="status" name="status" value="3">
							...
							<input type="text" name="eresid" value="${param.eresid}" style="display:none">
							<div class="form-group" id="noteDiv">
								<!-- 定义表单提示文字 -->
								<label class="col-md-2 control-label" for="note">审核备注：</label>
								<div class="col-md-6">
									<!-- 定义表单输入组件 -->
									<textarea id="renote" name="renote" rows="3"
										class="form-control" placeholder="请输入审核所给出的意见信息" rows="10"></textarea>
								</div>
								<!-- 定义表单错误提示显示元素 -->
								<div class="col-md-3" id="renoteMsg"></div>
							</div> 
