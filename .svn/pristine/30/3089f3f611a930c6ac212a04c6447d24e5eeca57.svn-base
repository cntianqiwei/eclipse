...
public class ResourceDAOImpl extends AbstractDAO implements IResourceDAO {
	...
	public List<Resource> findByEresid(Long eresid) throws SQLException {
		...
		String sql = "SELECT resid,spid,title,price,appdate,amount,photo FROM resource WHERE resid IN ("
				+ " SELECT resid FROM emp_resource_details WHERE eresid=?)";
	...
	public List<Resource> findBySpid(Long spid) throws SQLException {
		List<Resource> all = new ArrayList<>();
		String sql = "SELECT resid,spid,title,price,appdate,amount,photo,item,note FROM resource WHERE spid=?";
		super.pstmt=super.conn.prepareStatement(sql);
	...
	public boolean doCreate(Resource vo) throws SQLException {
		String sql = "INSERT INTO resource (spid,title,price,appdate,amount,photo,item,note)VALUES(?,?,?,?,?,?,?,?)";
		super.pstmt=super.conn.prepareStatement(sql);
	...
	public boolean doEdit(Resource vo) throws SQLException {
		String sql = "UPDATE resource SET title=?,price=?,amount=?,photo=?,note=?,item=? WHERE resid=?";
		super.pstmt=super.conn.prepareStatement(sql);
		super.pstmt.setString(1, vo.getTitle());
		super.pstmt.setDouble(2, vo.getPrice());
		super.pstmt.setInt(3, vo.getAmount());
	...
	public Resource findById(Long id) throws SQLException {
		String sql = "SELECT resid,spid,title,price,appdate,amount,photo,note,item FROM resource WHERE resid=?";
		super.pstmt=super.conn.prepareStatement(sql);
