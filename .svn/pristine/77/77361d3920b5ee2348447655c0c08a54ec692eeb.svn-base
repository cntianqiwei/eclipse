$(function(){
	$("span[id^=currnum-]").each(function(){
			did=this.id.split("-")[1];
			maxnum=$("#maxnum-"+did).val();
			currnum=$(this).text();
			if(parseInt(currnum)>=parseInt(maxnum)){
	...
	$("button[id^=edit-]").each(function(){
		if(!($(this).attr("disabled"))){
			$(this).on("click",function(){
				...
				maxnum=$("#maxnum-"+did).val();
				if (dname == ""||maxnum=="") { 
					operateAlert(false,"","部门名称和人数上限不允许为空，请确认后再提交更新！") ;
				} else {
					$.ajax({
						url:"/eop/pages/back/admin/dept/dept_edit.action",
						data:{
							"dname":dname,
							"maxnum":maxnum,
							"did":did
						...
						success:function(data){
							if(data.trim()=="true"){
								operateAlert(true,"部门信息修改成功","") ;
							}else{
								operateAlert(false,"","部门信息修改失败") ;
							}
							
						},
						error:function(data){
							operateAlert(false,data,"部门信息修改失败") ;
						}
					})
	...
	$("span[id^=eid-]").each(function(){
		$(this).on("click",function(){
			...
			$.ajax({
				...
				success:function(data){
					console.log(data);
					hiredate=data.emp.hiredate;
					hiredate=new Date(hiredate).format("yyyy-MM-dd");
