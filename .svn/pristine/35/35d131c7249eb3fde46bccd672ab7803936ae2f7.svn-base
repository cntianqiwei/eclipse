...
/**
 * 跳转操作方法，jump_url需自行定义，跳转秒数的元素ID应为"jump"
 * @returns 跳转到jump_url页面
 */
function jump(){
	eleObject=document.getElementById("jump");
	time=parseInt(eleObject.innerHTML);
	if(time==0){
		window.location=jump_url;
	}else{
		time--;
		eleObject.innerHTML=time;
