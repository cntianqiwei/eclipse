...
<%!
	...
	public static final String PLAN_PREPARE_LIST_URL = "pages/back/admin/plan/plan_prepare_list.action" ;
	//public static final String PLAN_HISTORY_LIST_URL = "pages/back/admin/plan/plan_history_list.jsp" ;
	public static final String PLAN_HISTORY_LIST_URL = "pages/back/admin/plan/plan_history_list.action" ;
	
	public static final String SUPPLY_ADD_URL = "pages/back/admin/supply/supply_apply_add.jsp" ;
	public static final String SUPPLY_APPLY_LIST_URL = "pages/back/admin/supply/supply_apply_list.action" ;
	public static final String SUPPLY_LIST_URL = "pages/back/admin/supply/supply_resource_list.action" ;
	//public static final String SUPPLY_RESOURCE_LIST_URL = "pages/back/admin/supply/supply_emp_apply_list.jsp" ;
	public static final String SUPPLY_RESOURCE_LIST_URL = "pages/back/admin/supply/supply_emp_apply_list.action" ;
	...
	<section class="sidebar">
		...
		<ul class="sidebar-menu">
			<li class="header"><i class="fa fa-university"></i> MLDN办公平台</li>
					<c:if test="${fn:contains(privilege.allRoles,'chief') || fn:contains(privilege.allRoles,'dept')}">
						<li class="treeview ${param.mi==1 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
								class="fa fa-institution"></i> <span>部门管理</span> <i
								class="fa fa-angle-left pull-right"></i>
						</a>
						<ul class="treeview-menu">
								<c:if test="${fn:contains(privilege.allActions, 'dept:add') }">
									<li class="${param.msi==11 ? 'active' : ''}"><a href="<%=DEPT_ADD_URL%>"><i class="fa fa-cube"></i>
									增加部门</a></li>
								</c:if>
								<c:if test="${fn:contains(privilege.allActions, 'dept:edit') || fn:contains(privilege.allActions, 'dept:list') || fn:contains(privilege.allActions, 'chief:deptlist')}">
					...
					<c:if test="${fn:contains(privilege.allRoles, 'chief') || fn:contains(privilege.allRoles, 'emp')}">
						<li class="treeview ${param.mi==2 ? 'active' : ''}"><a href="${basePath}<%=INDEX_URL%>"> <i
							class="fa fa-sitemap"></i> <span>人事管理</span> <i
							class="fa fa-angle-left pull-right"></i>
						</a>
						<ul class="treeview-menu">
						<c:if test="${fn:contains(privilege.allRoles, 'emp') }">
					...
					<c:if test="${fn:contains(privilege.allRoles, 'reimbursement') }">
					...
					<c:if test="${fn:contains(privilege.allRoles, 'resource') }">
					...
					<c:if test="${privilege.allRoles != null }">
					...
					<c:if test="${fn:contains(privilege.allRoles, 'finance') }">
					...
					<c:if test="${fn:contains(privilege.allRoles, 'supply') }">
